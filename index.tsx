...
export const state = {
  ...
  config: {
    ...
    sourceValue: '',
    platforms: ['android'],
    plugins: ['cordova-plugin-device', 'cordova-plugin-statusbar'],
    pluginVariables: {},
    enableGit: true,
    adConfig: {
        enabled: false,
...
document.addEventListener('deviceready', async () => {
    ...
    const generatePluginXml = (p: string) => {
        if (p === 'admob-plus-cordova') return '';
...
<widget id="${state.config.id || 'com.example.app'}" version="${state.config.version}" xmlns="http://www.w3.org/ns/widgets">
    <name>${state.config.name || 'CordovaApp'}</name>
    <description>${state.config.description || 'Generated by AltoGen'}</description>
    ...
    const virtualPackageJson = JSON.stringify({
        name: (state.config.id || 'com-example-app').toLowerCase().replace(/\./g, '-'),
        displayName: state.config.name || 'CordovaApp',
...
export const fileActions = {
  ...
  renameFile: (oldPath: string, newPath: string) => {
    // Check if new path collision exists
    const collision = state.files.find(f => f.path === newPath && !f.isVirtual);
    if (collision) {
        showToast("Path already exists", "error");
        return;
    }

    // Update all files sharing the prefix (recursively updates folders)
    ...
    // Handle updating editor path if it's currently open
    if (state.editingPath === oldPath) {
        state.editingPath = newPath;
    } else if (state.editingPath?.startsWith(oldPath + '/')) {
        const relative = state.editingPath.substring(oldPath.length);
        state.editingPath = newPath + relative;
    }
...
const init = () => {
    ...
    if (config) {
        ...
        if (state.config.enableGit === undefined) state.config.enableGit = true;
